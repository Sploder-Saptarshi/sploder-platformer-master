package com.sploder.builder.ui {
	...
	public class DialogueMusicManager extends Dialogue {
		...
		public function createContent():void 
		{
			...
			_nameField = new FormField(null, "None selected...", 292, 30, true, new Position( { margin_top: 10 } ));
			_nameField.x = 20;
			_nameField.y = 285;
        ...
        protected function addItem (xmlRef:XMLNode, itemtype:String, num:int):CollectionItem {
			...
			var title:String = com.sploder.util.StringUtils.titleCase(unescape(xmlRef.attributes.title).split(".mp3").join("").split("-").join(" "));
		...
		protected function loadSongReady ():void {
			...
			if (CreatorMain.dataLoader && _currentMusicTrack && _items[_currentMusicTrack]) {
				...
				trace(CreatorMain.dataLoader.baseURL + "/music/modules/" + _items[_currentMusicTrack].reference.url);
...
while (getTimer() - startTime < delay) {
    ...
    // Restart the song for repeating
    channel = song.play();
    channel.addEventListener(Event.SOUND_COMPLETE, onSoundComplete);
}
