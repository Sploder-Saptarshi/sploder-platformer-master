...
package fuz2d.screen {
	...
	public class BitView extends View {
		...
		//
		//
		public function BitView (main:Object, model:Model, camera:Camera2d, viewRange:Number = 800, width:Number = 0, height:Number = 0, x:Number = 0, y:Number = 0, roughSort:Boolean = true, container:Sprite = null, pixelSnap:Boolean = false, fastDraw:Boolean = false, isMain:Boolean = false) {
	
			super(main, model, camera, viewRange, width, height, x, y, true, container, true, true, isMain);
		...
		override protected function init (main:Object, model:Model, camera:Camera2d, viewRange:Number = 800, width:Number = 0, height:Number = 0, x:Number = 0, y:Number = 0, container:Sprite = null, pixelSnap:Boolean = false, fastDraw:Boolean = false, isMain:Boolean = false):void {
			...
			_pixelSnap = _camera.pixelSnap = _fastDraw = true;
		...
		override protected function render(object:Object2d = null):void {
			...
			m.a = m.d = View.scale / _pixelScale;
			m.b = m.c = 0;
			m.tx = m.ty = 0;
			...
			for (var i:int = 0; i < v; i++) {
				...
				if (_objectSprites[obj] != null) {
					...
					if (sp != null) {
						...
						loc.x = Math.round(reg.x / _pixelScale);
						loc.y = Math.round(reg.y / _pixelScale);
						loc.x += Math.ceil(sp.x * View.scale / _pixelScale);
						loc.y += Math.ceil(sp.y * View.scale / _pixelScale);
						...
						if (src != null && (!obj.controlled || obj is Tile)) {
							...
							if (vo) {
								if (vo.polygon && vo.polygon.cacheAsBitmap) {
									m.a = m.d = 1 / View.scale / _pixelScale;
								} else {
									m.a = m.d = View.scale / _pixelScale;
								}
								m.b = m.c = 0;
