package fuz2d.action.control.ai {
	...
	*/
	public class FlyingAllyController extends PlayObjectController {
		...
		//
		//
		public function FlyingAllyController (object:PlayObjectControllable, speed:int = 5, aggression:int = 50, weaponsRange:int = 0, projectile:String = "", fireDelay:int = 0, firePower:int = 100, bank:Boolean = true) {
		...
		override public function see(p:PlayObject):void {
			...
			if (_target == null)
			{
				...
				{
					if (pobj.group == "evil" && _object.playfield.map.canSee(_object, pobj))
					{
						_target = pobj;
						_targetNear = true;
						break;
		...
		override public function update (e:Event):void {
			...
			if (_firstSight > 0 && TimeStep.realTime - _firstSight > 180000)
			{
			...
			if (_player != null && _player.deleted) {
				...
				sqdist = Geom2d.squaredDistanceBetweenPoints(_object.object.point, _target.object.point);
		...
		public function get canAttack ():Boolean  {
			...
			if (_fireDelay > 0 && (TimeStep.realTime - _lastAttack > _fireDelay)) {
		...
		protected function onCollision (e:CollisionEvent):void {
			...
			if (_target == null && e.collider.group == "evil") {
